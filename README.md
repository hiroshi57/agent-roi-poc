...
**因果推論ROI** — 汎用BIが出せない「相関でなく因果」でAI導入効果を主張する:
- **合成対照法(Synthetic Control)**: 複数の対照部門を重み付き合成し「導入しなければどうだったか」の反実仮想を構築（`src/causal/`）
- **DID（差分の差）** ＋ **プラセボ検定**で効果の頑健性を検証（偽の介入では効果が出ないことを確認）
- 経営が「AI投資は本当に効いたか」を意思決定できる唯一のROIツールを志向

加えて「導入後は速くなりました」で終わらせないための2点:
